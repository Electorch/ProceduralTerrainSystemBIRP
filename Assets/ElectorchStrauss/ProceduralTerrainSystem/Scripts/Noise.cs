...
namespace ElectorchStrauss.ProceduralTerrainSystem.Scripts
{
	public static class Noise
	{
		...
		public static float[,] GenerateNoiseMap(int mapSize, int seed, float scale, int octaves, float persistance,
			float lacunarity, Vector2 offset, NormalizeMode normalizeMode)
		{
			...
			for (int y = 0; y < mapSize; y++)
			{
				for (int x = 0; x < mapSize; x++)
				{
					...
					{
						float normalizedHeight = (noiseMap[x, y] + 1) / (maxPossibleHeight / 0.666f);
